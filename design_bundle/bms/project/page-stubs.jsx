...
const CompanyAccessSummary = ({ user }) => {
  if (user.companies === "all") {
    return <span className="badge badge-navy"><span className="dot"></span>ทุกบริษัท</span>;
  }
  const ids = user.companies || [];
  if (ids.length === 0) {
    return <span className="text-muted text-xs">ยังไม่กำหนด</span>;
  }
  const codes = ids.map(id => (TRANSPORT_COMPANIES.find(c => c.id === id) || {}).code).filter(Boolean);
  const shown = codes.slice(0, 3);
  const rest  = codes.length - shown.length;
  return (
    <div className="access-chips">
      {shown.map(code => <span key={code} className="company-chip mono">{code}</span>)}
      {rest > 0 && <span className="text-muted text-xs" style={{marginLeft:2}}>+{rest}</span>}
    </div>
  );
};
...
const Users = ({ onToast }) => {
  const initialUsers = [
    { id: 1, name: "พรทิพย์ น.",      email: "pornthip.n@bms.co.th",   role: "admin",     companies: "all",                          last: "วันนี้ 14:22" },
    { id: 2, name: "ธนพล ส.",        email: "thanapol.s@bms.co.th",   role: "admin",     companies: "all",                          last: "วันนี้ 11:08" },
    { id: 3, name: "สมหญิง พ.",      email: "somying.p@bms.co.th",    role: "operation", companies: ["TC001", "TC002", "TC004"],     last: "วันนี้ 09:45" },
    { id: 4, name: "วราภรณ์ ก.",     email: "waraporn.k@bms.co.th",   role: "operation", companies: ["TC002", "TC003"],              last: "เมื่อวานนี้ 17:20" },
    { id: 5, name: "ภาณุพงษ์ ม.",    email: "panupong.m@bms.co.th",   role: "operation", companies: ["TC005", "TC006", "TC007"],     last: "เมื่อวานนี้ 14:50" },
    { id: 6, name: "นพดล จ.",        email: "noppadol.j@bms.co.th",   role: "operation", companies: ["TC001"],                       last: "3 วันที่แล้ว" },
  ];
  ...
  const [form, setForm] = useState({ name: "", email: "", role: "operation", companies: [], allCompanies: false, password: "", confirm: "" });
  const [errors, setErrors] = useState({});

  const openAdd = () => {
    setForm({ name: "", email: "", role: "operation", companies: [], allCompanies: false, password: "", confirm: "" });
    setErrors({});
    setEditing(null);
    setModalMode("add");
  };
  ...
  const validate = () => {
    ...
    if (form.role === "operation" && !form.allCompanies && form.companies.length === 0) {
      e.companies = "กรุณาเลือกบริษัทขนส่งอย่างน้อย 1 บริษัท";
    }
    setErrors(e);
  ...
  const save = () => {
    if (!validate()) return;
    const companiesVal = form.role === "admin"
      ? "all"
      : (form.allCompanies ? "all" : form.companies.slice());
    if (modalMode === "add") {
      const newU = {
      ...
      setUsers(users.map(u => u.id === editing.id ? { ...u, name: form.name.trim(), email: form.email.trim(), role: form.role, companies: companiesVal } : u));
  ...
  return (
    <div>
      ...
      <div className="card">
        <div className="tbl-wrap">
          <table className="tbl">
            <thead><tr><th>ชื่อ</th><th>อีเมล</th><th>บทบาท</th><th>สิทธิ์บริษัทขนส่ง</th><th>เข้าใช้ระบบล่าสุด</th><th style={{textAlign:"right"}}>การจัดการ</th></tr></thead>
            <tbody>
              {users.map((u) => (
                <tr key={u.id}>
                  <td>
                  ...
                  <td><CompanyAccessSummary user={u}/></td>
              ...
              {users.length === 0 && (
                <tr><td colSpan="6" style={{textAlign:"center", padding:"40px 0", color:"var(--text-muted)"}}>ยังไม่มีผู้ใช้</td></tr>
              )}
            </tbody>
          </table>
        </div>
              ...
              <div className="field">
                ...
                <select
                  ...
                  onChange={e => {
                    ...
                    setForm({ ...form, role, allCompanies: role === "admin" ? true : form.allCompanies });
                  }}
                >
                  <option value="admin">Admin — เข้าถึงทุกเมนู รวมถึงการจัดการผู้ใช้</option>
                  <option value="operation">Operation — ใช้งานทั่วไป (ไม่เห็นเมนู User Management)</option>
                </select>
              </div>

              <div className="field">
                <label>สิทธิ์เข้าถึงข้อมูลบริษัทขนส่ง *</label>
                {form.role === "admin" ? (
                  <div className="access-locked">
                    <Icon name="shield" size={14} style={{color:"var(--navy-700)"}}/>
                    <span>Admin เข้าถึงข้อมูลของทุกบริษัทโดยอัตโนมัติ</span>
                  </div>
                ) : (
                  <>
                    <label className="access-all-toggle">
                      <input
                        type="checkbox"
                        checked={form.allCompanies}
                        onChange={e => setForm({ ...form, allCompanies: e.target.checked, companies: e.target.checked ? [] : form.companies })}
                      />
                      <div>
                        <div className="access-all-label">ทุกบริษัทขนส่ง</div>
                        <div className="access-all-desc">ผู้ใช้สามารถเห็นข้อมูลของทุกบริษัทในระบบ</div>
                      </div>
                    </label>
                    <div className={"company-access-grid " + (form.allCompanies ? "is-disabled" : "")}>
                      {TRANSPORT_COMPANIES.map(c => {
                        const checked = form.allCompanies || form.companies.includes(c.id);
                        return (
                          <label key={c.id} className={"company-access-opt " + (checked ? "on" : "")}>
                            <input
                              type="checkbox"
                              disabled={form.allCompanies}
                              checked={checked}
                              onChange={e => {
                                const next = e.target.checked
                                  ? [...form.companies, c.id]
                                  : form.companies.filter(x => x !== c.id);
                                setForm({ ...form, companies: next });
                              }}
                            />
                            <span className="company-access-code mono">{c.code}</span>
                            <span className="company-access-name" title={c.name}>{c.name}</span>
                          </label>
                        );
                      })}
                    </div>
                    {errors.companies && <div className="field-err">{errors.companies}</div>}
                    {!form.allCompanies && (
                      <div className="text-xs text-muted mt-4">
                        เลือกแล้ว {form.companies.length} / {TRANSPORT_COMPANIES.length} บริษัท
                      </div>
                    )}
                  </>
                )}
