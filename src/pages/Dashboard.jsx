...
export default function Dashboard() {
  ...
  const fanData = FUEL_BY_COMPANY.map(c => ({
    value: c.liters,
    label: c.name.replace(' ทรานสปอร์ต', '').replace(' ทรานส์', '').replace('ขนส่ง', ''),
  }))
  ...
  return (
    <div>
      ...
      <div className="kpi-grid">
        <KPI tone="navy"  icon="bus"    label="รถบัสที่ใช้งาน"    value={`${BUSES_ACTIVE} / 88`}  trend={{ dir: 'up',      label: '+2 vs สัปดาห์ก่อน' }} />
        <KPI tone="amber" icon="wrench" label="อยู่ระหว่างซ่อม"   value={String(MAINTENANCE_NOW)} trend={{ dir: 'down',    label: '-1 vs สัปดาห์ก่อน' }} />
        <KPI tone="blue"  icon="coin"   label="ค่าใช้จ่ายเดือนนี้" value={bahtCompact(COST_THIS_MONTH)} trend={{ dir: 'up', label: '+5.8% MoM' }} />
        <KPI tone="red"   icon="alert"  label="เรื่องร้องเรียนค้าง" value={String(COMPLAINTS_OPEN)} trend={{ dir: 'neutral', label: 'คงที่' }} />
      </div>
      ...
      <div className="mt-16" style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 16 }}>
        <div className="card">
          <div className="card-head">
            ...
            <div className="flex items-center gap-8" style={{ fontSize: 12, color: 'var(--success)' }}>
              <span style={{ width: 8, height: 8, borderRadius: 8, background: 'var(--success)', display: 'inline-block' }}></span>
              <span>LIVE</span>
            </div>
          </div>
          <div className="card-body">
            <BarChart data={fanData} valueKey="value" labelKey="label" color="#1e3a5f" suffix=" ล." height={260} />
          </div>
        </div>
        ...
        <div className="card">
          <div className="card-head">
            ...
            <button className="btn btn-ghost btn-sm">ทั้งหมด</button>
          </div>
