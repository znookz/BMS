/* ============================
   BMS — Charts
   Custom SVG: LineChart, BarChart, DonutChart
   ============================ */

const LineChart = ({ data, height = 220, lines = [] }) => {
  // data: [{x, ...keys}]; lines: [{key, label, color}]
  ...
  const gridLines = Array.from({length: grid + 1}, (_, i) => {
    ...
    const y = py(v);
    return { y, label: v >= 1000 ? (v/1000).toFixed(1) + "M" : v };
  });
        ...
        {lines.map(l => {
          ...
          return (
            <g key={l.key}>
              ...
              {data.map((d, i) => (
                <circle key={i} cx={px(i)} cy={py(d[l.key])} r="3" fill="#fff" stroke={l.color} strokeWidth="1.6" />
              ))}
            </g>
          );
        })}
...
const BarChart = ({ data, height = 240, valueKey = "value", labelKey = "label", color = "var(--navy-700)", suffix = "" }) => {
